...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:#0a0a0a;color:var(--paper);font-family:"BDO Grotesk Variable",Inter,Arial,sans-serif;-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision}
    body:before{content:"";position:fixed;inset:0;z-index:-2;background:linear-gradient(180deg,#0e1416 0%,#0a0a0a 38%,#0a0a0a)}
    ...
    .section-head{display:grid;grid-template-columns:1fr auto;align-items:end;gap:20px;padding:96px 0 22px;border-bottom:1px solid var(--line)}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;padding:28px 0 120px}.card{border-top:1px solid var(--line);padding-top:14px;position:relative;animation:riseIn 1.2s cubic-bezier(.16,1,.3,1) both}.thumb-frame{aspect-ratio:16/9;overflow:hidden;background:#151515;position:relative}.thumb{width:100%;height:100%;object-fit:cover;display:block;transform:scale(1.01);transition:transform .8s cubic-bezier(.16,1,.3,1),filter .8s}.thumb-video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .5s}.card:hover .thumb{transform:scale(1.055);filter:brightness(.68)}.card:hover .thumb-video{opacity:1}.meta{display:flex;align-items:center;justify-content:space-between;gap:12px;color:var(--muted);font-size:12px;text-transform:uppercase;margin:12px 0 18px}.card h3{font-weight:400;font-size:clamp(34px,4.6vw,62px);letter-spacing:-.075em;line-height:.94;margin:0 0 6px}
    ...
    @keyframes riseIn{from{opacity:0;transform:translateY(48px)}to{opacity:1;transform:translateY(0)}}@keyframes posterIn{from{opacity:.001;transform:scale(1.08)}to{opacity:.48;transform:scale(1.04)}}@media(max-width:920px){.topbar{bottom:14px}.topbar .wrap{width:calc(100vw - 28px);justify-content:space-between}.hero-row,.grid,.split,.admin-grid,.row{grid-template-columns:1fr}.copy-large{margin:22px 0 0;text-align:left}.timeline{grid-template-columns:repeat(3,1fr)}.item{grid-template-columns:82px 1fr}.item form{grid-column:1/-1}h1{font-size:clamp(68px,24vw,132px)}}
  </style>
