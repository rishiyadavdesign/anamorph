...
function adminPage(data) {
  ...
  return shell("Anamorph CMS", `<header class="topbar"><div class="wrap"><a class="brand" href="/">Anamorph</a><nav class="nav"><a class="pill" href="/work">View Work</a><a class="pill" href="/reels">View Reels</a><form method="post" action="/logout"><button>Logout</button></form></nav></div></header><main class="wrap admin-grid"><section><div class="eyebrow">(CMS) - Local Content</div><h2>Projects</h2><p>Change project pages, posters, videos, copy, and publish state.</p><div class="list">${items || "<p>No projects yet.</p>"}</div><div style="margin-top:24px">${edits}</div><div class="section-head"><div><div class="eyebrow">(CMS) - Reels</div><h2>Reels</h2></div></div><div class="list">${reelItems || "<p>No reels yet.</p>"}</div><div style="margin-top:24px">${reelEdits}</div></section><aside class="panel glass"><div class="eyebrow">(Upload) - New Case</div><h2>Project</h2>${projectForm()}<div style="height:30px"></div><div class="eyebrow">(Upload) - New Reel</div><h2>Reel</h2>${reelForm()}</aside></main>`);
}
