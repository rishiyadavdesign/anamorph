...
function homeForm(home = {}) {
  return `<form class="cms-form" method="post" action="/admin/home" enctype="multipart/form-data"><div class="cms-editor-head"><div><div class="eyebrow">Guided home editor</div><h2>Content Studio</h2><p>Scan the live homepage, choose the exact section, then replace one text or image without touching the Framer layout.</p></div><div><button class="primary" type="button" data-scan-home>Scan Homepage</button><div class="scan-status" data-scan-status>Ready to scan</div></div></div><div class="home-editor-grid"><section class="home-tool-card full"><div class="cms-fieldset-title"><span>00</span><strong>Homepage sections</strong></div><label>Filter by section<select id="homeSectionFilter"><option value="">All homepage sections</option></select></label><div id="homeSectionMap" class="home-section-map"></div></section><section class="home-tool-card"><div><div class="eyebrow">01 - Text</div><h3>Replace Copy</h3><p>Pick existing homepage text, then type the new text.</p></div><label>Current homepage text<input name="text_from" list="homeTextOptions" placeholder="Choose or paste current text"></label><label>New text<input name="text_to" placeholder="Type replacement text"></label></section><section class="home-tool-card"><div><div class="eyebrow">02 - Image</div><h3>Swap Visual</h3><p>Choose an existing image, then upload a replacement or paste a Google Drive image link.</p></div><label>Current image URL<input name="image_target" list="homeImageOptions" placeholder="Choose or paste current image URL"></label><label>New Google Drive image URL<input name="image_url" placeholder="Paste Drive image share link"></label><label>Or upload replacement image<input type="file" name="image" accept="image/*"></label></section><section class="home-tool-card full"><div class="cms-fieldset-title"><span>03</span><strong>Saved replacements</strong></div><div class="saved-replacements"><label>All text replacements<textarea name="text_replacements" placeholder="Anamorph => Your Brand&#10;Book a call => Start a project">${escapeHtml(pairsToText(home.text_replacements || []))}</textarea></label><label>All image replacements<textarea name="image_replacements" placeholder="/assets/local/323795fc9c20f1ac.png => https://drive.google.com/file/d/.../view">${escapeHtml(pairsToText(home.image_replacements || []))}</textarea></label></div></section></div><datalist id="homeTextOptions"></datalist><datalist id="homeImageOptions"></datalist><input type="submit" value="Save home page"><script>(function(){var state={texts:[],images:[],sections:[]};function clean(v){return (v||'').replace(/\s+/g,' ').trim();}function status(v){var el=document.querySelector('[data-scan-status]');if(el)el.textContent=v;}function sectionName(el){var section=el&&el.closest&&el.closest('section,[data-framer-name],footer,header');if(!section)return 'Global';return section.getAttribute('data-framer-name')||section.id||section.tagName||'Global';}function uniqueItems(items){var seen={};return items.filter(function(item){var key=item.section+'::'+item.value;if(seen[key]||!item.value)return false;seen[key]=1;return true;}).slice(0,500);}function filtered(items){var section=document.getElementById('homeSectionFilter')?.value||'';return section?items.filter(function(item){return item.section===section;}):items;}function fill(list,items){var el=document.getElementById(list);if(!el)return;el.innerHTML='';filtered(items).forEach(function(item){var option=document.createElement('option');option.value=item.value;option.label=item.section;el.appendChild(option);});}function renderSections(){var select=document.getElementById('homeSectionFilter');var map=document.getElementById('homeSectionMap');if(!select||!map)return;select.innerHTML='<option value="">All homepage sections</option>';state.sections.forEach(function(section){var option=document.createElement('option');option.value=section;option.textContent=section;select.appendChild(option);});map.innerHTML='';state.sections.forEach(function(section){var textCount=state.texts.filter(function(item){return item.section===section;}).length;var imageCount=state.images.filter(function(item){return item.section===section;}).length;var button=document.createElement('button');button.type='button';button.className='home-section-chip';button.innerHTML='<strong>'+section+'</strong><span>'+textCount+' text / '+imageCount+' image</span>';button.addEventListener('click',function(){select.value=section;refresh();});map.appendChild(button);});}function refresh(){fill('homeTextOptions',state.texts);fill('homeImageOptions',state.images);}async function scan(){status('Scanning homepage');try{var res=await fetch('/',{cache:'no-store'});var html=await res.text();var doc=new DOMParser().parseFromString(html,'text/html');var blocked={SCRIPT:1,STYLE:1,NOSCRIPT:1};var texts=[];var walker=doc.createTreeWalker(doc.body,NodeFilter.SHOW_TEXT,{acceptNode:function(node){var parent=node.parentElement;if(!parent||blocked[parent.tagName])return NodeFilter.FILTER_REJECT;var text=clean(node.nodeValue);return text.length>1&&text.length<220?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT;}});while(walker.nextNode()){texts.push({value:clean(walker.currentNode.nodeValue),section:sectionName(walker.currentNode.parentElement)});}var images=[];doc.querySelectorAll('img').forEach(function(img){var section=sectionName(img);var src=img.getAttribute('src');if(src)images.push({value:src,section:section});(img.getAttribute('srcset')||'').split(',').forEach(function(part){var first=part.trim().split(/\s+/)[0];if(first)images.push({value:first,section:section});});});state.texts=uniqueItems(texts);state.images=uniqueItems(images);state.sections=Array.from(new Set(state.texts.concat(state.images).map(function(item){return item.section;}))).filter(Boolean);renderSections();refresh();status(state.sections.length+' sections scanned');}catch(e){status('Scan failed - refresh and try again');}}document.getElementById('homeSectionFilter')?.addEventListener('change',refresh);document.querySelector('[data-scan-home]')?.addEventListener('click',scan);scan();})();</script></form>`;
}
