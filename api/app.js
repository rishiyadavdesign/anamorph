...
function homeForm(home = {}) {
  return `<form class="cms-form" method="post" action="/admin/home" enctype="multipart/form-data"><div class="cms-editor-head"><div><div class="eyebrow">Guided home editor</div><h2>Text & Images</h2><p>Pick anything currently on the homepage, add the new value, then save.</p></div><button class="primary" type="button" data-scan-home>Scan Homepage</button></div><datalist id="homeTextOptions"></datalist><datalist id="homeImageOptions"></datalist><div class="cms-fieldset"><div class="cms-fieldset-title"><span>01</span><strong>Change one text</strong></div><div class="row"><label>Current homepage text<input name="text_from" list="homeTextOptions" placeholder="Choose or paste current text"></label><label>New text<input name="text_to" placeholder="Type replacement text"></label></div></div><div class="cms-fieldset"><div class="cms-fieldset-title"><span>02</span><strong>Change one image</strong></div><div class="row"><label>Current image URL<input name="image_target" list="homeImageOptions" placeholder="Choose or paste current image URL"></label><label>New Google Drive image URL<input name="image_url" placeholder="Paste Drive image share link"></label></div><label>Or upload replacement image<input type="file" name="image" accept="image/*"></label></div><details class="cms-advanced" open><summary>Advanced saved replacements</summary><label>All text replacements<textarea name="text_replacements" placeholder="Anamorph => Your Brand&#10;Book a call => Start a project">${escapeHtml(pairsToText(home.text_replacements || []))}</textarea></label><label>All image replacements<textarea name="image_replacements" placeholder="/assets/local/323795fc9c20f1ac.png => https://drive.google.com/file/d/.../view">${escapeHtml(pairsToText(home.image_replacements || []))}</textarea></label></details><input type="submit" value="Save home page"><script>(function(){function unique(values){return Array.from(new Set(values.map(function(v){return (v||'').trim();}).filter(function(v){return v.length>1;}))).slice(0,350);}function fill(list,values){var el=document.getElementById(list);if(!el)return;el.innerHTML='';unique(values).forEach(function(value){var option=document.createElement('option');option.value=value;el.appendChild(option);});}async function scan(){try{var res=await fetch('/',{cache:'no-store'});var html=await res.text();var doc=new DOMParser().parseFromString(html,'text/html');var blocked={SCRIPT:1,STYLE:1,NOSCRIPT:1};var texts=[];var walker=doc.createTreeWalker(doc.body,NodeFilter.SHOW_TEXT,{acceptNode:function(node){var parent=node.parentElement;if(!parent||blocked[parent.tagName])return NodeFilter.FILTER_REJECT;var text=node.nodeValue.replace(/\\s+/g,' ').trim();return text.length>1&&text.length<220?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT;}});while(walker.nextNode())texts.push(walker.currentNode.nodeValue.replace(/\\s+/g,' ').trim());var images=[];doc.querySelectorAll('img').forEach(function(img){var src=img.getAttribute('src');if(src)images.push(src);(img.getAttribute('srcset')||'').split(',').forEach(function(part){var first=part.trim().split(/\\s+/)[0];if(first)images.push(first);});});fill('homeTextOptions',texts);fill('homeImageOptions',images);}catch(e){}}document.querySelector('[data-scan-home]')?.addEventListener('click',scan);scan();})();</script></form>`;
}
